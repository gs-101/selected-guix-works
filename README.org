...
- ~local-file-from-env~ ::
  ...
  Wrapper that enabled the use of environment variables with ~local-file~. The first argument given is the environment to be used, the second the path /starting from the environment variable/ and, like the regular ~local-file~, there's an optional parameter for a file name, and a keyword parameter telling if the file should be added recursively or not (which chould indicate a directory instead.

  In this example, the configuration file for Nix is referenced:

  #+begin_src scheme
    (local-file-from-env "XDG_CONFIG_HOME" "/nix/nix.conf")
  #+end_src

  This function was written because ~local-file~ doesn't support environment variables on its path.
