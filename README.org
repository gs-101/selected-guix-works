...
- ~home-ripgrep-service-type~ ::
  ...
  - Variable: ~home-ripgrep-configuration~ ::
    ...
    - ~search-zip?~ (type: maybe-boolean) ::
      ...
      Wether to search the inside compressed files.
    ...
    - ~mmap?~ (type: maybe-boolean) ::
      ...
      Wether to search using memory maps. =ripgrep= does this automatically, and it's useful for searching singile files.

    - ~multiline?~ (type: maybe-boolean) ::

      Wether to search across multiple lines. This allows for regualr expressions containing ‘\n’.
    ...
    - ~smart-case?~ (type: maybe-boolean) ::
      ...
      Wether to enable case-sensitive search only when a pattern character is capitalized.

    - ~text?~ (type: maybe-boolean) ::

      Wether to search binary files as if they were text. Beware, as this may end up passing escape sequences to your terminal.
    ...
    - ~word-regexp?~ (type: maybe-boolean) ::
      ...
      Wether to show only matches surrounded by word boundaries. This is the equivalent of surrounding every match with ‘\b{start-half}’ and ‘\b{end-half}’.

    - ~binary?~ (type: maybe-boolean) ::

      Wether to search binary files, without converting them to text. Stops search when a match is found.
    ...
    - ~one-file-system?~ (type: maybe-boolean) ::
      ...
      Wether to stop =ripgrep= from searching across different file systems.
    ...
    - ~line-number?~ (type: maybe-boolean) ::
      ...
      Wether to show line numbers. Enabled automatically with ~column?~.
    ...
    - ~max-columns-preview?~ (type: maybe-boolean) ::
      ...
      Wether to show a preview of matched lines over the limit of ~max-columns~, instead of removing the matched lines.

    - ~only-matching?~ (type: maybe-boolean) ::

      Wether to show only the matched parts of a matching line in a separate output line.
